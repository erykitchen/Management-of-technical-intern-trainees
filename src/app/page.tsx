  ...
  return (
    <main style={{ padding: '20px', backgroundColor: '#f0f2f5', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ padding: '10px 20px', backgroundColor: '#1a73e8', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>
          ＋ 新規実施者登録
        </button>
      </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#fff', width: '95%', maxWidth: '1000px', maxHeight: '90vh', overflowY: 'auto', padding: '30px', borderRadius: '12px' }}>
            <h2 style={{ marginTop: 0 }}>新規実習実施者登録</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '30px' }}>
              <div>
                ...
                <input style={inputStyle} type="text" onChange={e => setFormData({...formData, officeZip: e.target.value})} />
              ...
              <div>
                ...
                <select style={inputStyle} onChange={e => setFormData({...formData, acceptance: e.target.value})}>
                ...
                <input style={inputStyle} type="date" onChange={e => setFormData({...formData, investmentPayDate: e.target.value})} />
              </div>
              <div>
      ...
      <div style={{ display: 'grid', gap: '15px' }}>
        {companies.map(c => (
          <div key={c.id} style={{ backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #ddd', overflow: 'hidden' }}>
            ...
            {selectedId === c.id && (
              <div style={{ padding: '20px', backgroundColor: '#fafafa', borderTop: '1px solid #eee', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 20px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      {c[key] && (
                        <button 
                          onClick={() => copy(c[key])} 
                          title="コピー"
                          style={{ width: '22px', height: '22px', fontSize: '10px', cursor: 'pointer', backgroundColor: '#e1f5fe', border: '1px solid #03a9f4', borderRadius: '4px', color: '#01579b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        >
                          C
                        </button>
                      )}
